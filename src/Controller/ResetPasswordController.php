    ...
    public function index(Request $request): Response
    {
        ...
        if($request->get('email')){
            ...
            if($user){
                ...
                // 2 : Envoyer un email à l'utilisateur avec un lien permettant de mettre à jour le mot de pase
                $url = $this->generateUrl('update_password', [
                    'token' => $reset_password->getToken()
                ]);

                $title = 'Réinitaliser votre mot de passe sur la Boutique Française';
                $content = "Bonjour".$user->getFirstname()."<br> Vous avez demandé de mettre à jour votre mot de passe sur le site de La Boutique Française <br><br> ";
                $content .= "Merci de bien vouloir cliquer sur le lien suivant pour <a href='".$url."'>mettre à jour votre mot de passe</a>";

                $mail = new Mail();
                $mail->send($user->getEmail(), $user->getFirstname().''.$user->getLastname(), $title , $content);

                $this->addFlash('notice', 'Vous allez recevoir un email pour mettre à jour votre mot de passe');
